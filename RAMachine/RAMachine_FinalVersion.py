...
########## setting variables as symbols for equations ##############################
...
def RAMachine():
    ...
    def solve_force_eq_for(solve_var):
        randomized_dict_force = {}
        for var in (m1, m2, a):
            if var != solve_var:
                randomized_dict_force[var] = randomizer[var]()
        result = sympy.solve(force_eq.xreplace(randomized_dict_force), solve_var)
        force_result = result[0]
        return {solve_var: force_result}, randomized_dict_force


    ####### solve force function for acceleration ########################
    solved_dict_force, randomized_dict_force = solve_force_eq_for(a) 
    solution_a = solved_dict_force[a]
    randomized_m1 = randomized_dict_force[m1]
    randomized_m2 = randomized_dict_force[m2]
    ...
    def solve_tmax_eq_for(solve_var):
        randomized_dict_time = {}
        for var in (tmax, v0, x0):
            if var != solve_var:
                randomized_dict_time[var] = randomizer[var]()
        tmax_result_neg = sympy.solve(tmax_eq_neg.xreplace(randomized_dict_time), solve_var)
        tmax_result_pos = sympy.solve(tmax_eq_pos.xreplace(randomized_dict_time), solve_var)
        tmax_result = max(tmax_result_neg[0], tmax_result_pos[0])
        return {solve_var: tmax_result}, randomized_dict_time    



    ######## solve t_max function for t_max ################################
    solved_dict_time, randomized_dict_time = solve_tmax_eq_for(tmax)
    solution_tmax = solved_dict_time[tmax]
    randomized_v0 = randomized_dict_time[v0]
    randomized_x0 = randomized_dict_time[x0]
    ...
    print(f"Acceleration due to the planet's gravity (m/s^2): g = {g}")
    print(f"Range of masses for the left block (kg): [m1min = {m1min}, m1max = {m1max}]")
    print(f"Range of masses for the right block (kg): [m2min = {m2min}, m2max = {m2max}]")
    print(f"Range for initial velocity (m/s): [v0min = {v0min}, v0max = {v0max}]")
    print(f"Range for the length of the string (m): [1, Lmax = {Lmax}]")
    print('\n')
    
    print('###### RANDOMIZED VALUES ########\n')
    print(f'Mass of the left block (kg): randomized_m1 = {randomized_m1}')
    print(f'Mass of the right block (kg): randomized_m2 = {randomized_m2}')
    print(f'Initial velocity (m/s): randomized_v0 = {randomized_v0}')
    print(f"Range for initial position (m): [0, L)")
    print(f'Initial position (m): randomized_x0 = {randomized_x0}')
    print(f'Length of the string (m): L = {L}')
    print(f"Range for instantaneous time (s): [0, solution_tmax]")
    print(f'Instantaneous time (s): t = {t}')
    print('\n')
    
    print('###### SOLVED VALUES ########\n')
    print(f'Acceleration (m/s^2): solution_a = {solution_a}')
    print(f'Maximum time (s): solution_tmax = {solution_tmax}')
    print(f'Instantaneous velocity (m/s): v = {v}')
    print(f'Instantaneous position (m): x = {x}')
    print("\n")
